...
class Robar { 
    ...
    public: // Se declaran los metodos
         ...
         * @return Objeto Robar
         */
        Robar() : num1(0), num2(0), num3(0), intentos_maximos(6), multiplicador_premio(2) {};
...
void Robar::jugar(Jugador& player) {
    ...
    std::cout << "3. Boveda del Gobierno (Dificultad Dificil: 1 intento | Premio x4)\n";
    ...
    if (opcion_dificultad == 2) {
        ...
        if (opcion_dificultad != 1) {
            std::cout << "\nOpcion no valida. Seleccionando 'Casa Normal' por defecto]\n";
        } else {
    ...
    double apuesta;
    std::cout << "¿Cuanto dinero deseas invertir en las herramientas para el robo? (0 para salir): ";
    std::cin >> apuesta;
    ...
    limpiar_pantalla();
    std::cout << "\n[!] La caja fuerte tiene un PIN de 3 digitos (del 0 al 9).\n";
    std::cout << "[!] Los numeros SE PUEDEN REPETIR.\n";
    std::cout << " - \033[1;32mVERDE\033[0m: Numero correcto en la posicion correcta.\n";
    std::cout << " - \033[1;33mAMARILLO\033[0m: El numero esta en el PIN, pero en otra posicion.\n";
    std::cout << " - \033[1;31mROJO\033[0m: El numero no forma parte del PIN.\n";
    std::cout << "Tienes " << intentos_maximos << " intentos para adivinar el codigo correcto.\n";
    ...
    while (intentos_actuales > 0 && !abierta) {
        std::cout << "\n--- Intentos restantes: " << intentos_actuales << " ---\n";
    ...
    if (abierta) {
        ...
        std::cout << "La caja fuerte se ha abierto. ¡Robaste " << premio_total << " monedas!\n";
        player.sumar_monedas(premio_total); 
    } else {
        limpiar_pantalla();
        std::cout << "\n\033[1;31m[X] ALARMA ACTIVADA [X]\033[0m\n";
        std::cout << "El codigo correcto era: [ " << num1 << " " << num2 << " " << num3 << " ]\n";
        std::cout << "Afortunadamente lograste escapar, pero tuviste que dejar tus herramientas\n";
